...
Single-stage flow-matching **singing** synthesis (SVS) and conversion (SVC) @ 24 kHz via
`POST /v1/chat/completions` and `extra_body.extra_args` (not `/v1/audio/speech`).

Preprocess and weights: [offline SoulX-Singer hub](../../offline_inference/text_to_speech/README.md#soulx-singer).

### Launch

```bash
# assume user in project root
EXAMPLE_DIR=$(pwd)/tests/assets/soulxsinger

MODEL=/path/to/SoulX-Singer-svs \
  bash examples/online_serving/text_to_speech/soulxsinger/run_server.sh svs

MODEL=/path/to/SoulX-Singer-svc \
  bash examples/online_serving/text_to_speech/soulxsinger/run_server.sh svc
```

Default port is **8091** (`PORT` env). See the [offline hub](../../offline_inference/text_to_speech/README.md#soulx-singer) for the two-directory `config.json` layout.

### Sending requests

After preprocess (or bundled fixtures), paths in `extra_args` must exist on the **server** host:

```bash
# SVS
python examples/online_serving/text_to_speech/soulxsinger/openai_chat_client.py \
    --mode svs \
    --prompt-metadata-path $EXAMPLE_DIR/zh_prompt.json \
    --target-metadata-path $EXAMPLE_DIR/zh_target.json \
    --audio-path $EXAMPLE_DIR/zh_prompt.mp3 \
...
python examples/online_serving/text_to_speech/soulxsinger/openai_chat_client.py \
    --mode svc \
    --prompt-wav-path $EXAMPLE_DIR/zh_prompt.mp3 \
    --target-wav-path $EXAMPLE_DIR/music.mp3 \
    --prompt-f0-path $EXAMPLE_DIR/zh_prompt_f0.npy \
    --target-f0-path $EXAMPLE_DIR/music_f0.npy \
    --server http://127.0.0.1:8091 \
    -o output_svc.wav
```

Smoke test (server must see the same fixture paths):

```bash
python examples/online_serving/text_to_speech/soulxsinger/openai_chat_client.py \
    --mode svs --use-bundled-assets --server http://127.0.0.1:8091 -o svs.wav
```

### Notes

- Bind-mount shared storage if the client runs outside the server container.
- Long SVS uses multi-segment `target_metadata.json` in **one** request; do not stitch client-side.
- No streaming PCM; response is a full WAV in `message.audio`.
